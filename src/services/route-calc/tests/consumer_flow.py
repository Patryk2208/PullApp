...
@pytest.mark.parametrize("config", [
    {"thread_pool": {"max_workers": 4, "shutdown_timeout_seconds": 1}, "test": {"mock_queue": {"start_mode": "normal", "prefetch_count": 4}, "mock_algorithm": {"min_compute_time": 1, "max_compute_time": 4, "failure_probability": 0}, "run_time": 10, "stop_method": signal.SIGINT}},
    {"thread_pool": {"max_workers": 4, "shutdown_timeout_seconds": 1}, "test": {"mock_queue": {"start_mode": "normal", "prefetch_count": 4}, "mock_algorithm": {"min_compute_time": 0, "max_compute_time": 5, "failure_probability": 0.2}, "run_time": 5, "stop_method": signal.SIGTERM}},
...
async def test_consumer_flow(mock_queue_factory, mock_algorithm_factory, mock_compute_message_factory, config: dict):
    ...
    consumer = Consumer(config, mock_queue_factory(config["test"]["mock_queue"], possible_messages), mock_algorithm_factory(config["test"]["mock_algorithm"]))

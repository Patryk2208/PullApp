    ...
    def compute(self, payload: ComputeMessage) -> ResultMessage:
        ...
        with _tracer.start_as_current_span(
            ...
            if payload.algorithm == AlgorithmType.BEST_ROUTE:
                res = self._run_best_route(payload)
            elif payload.algorithm == AlgorithmType.CLOSEST_ROUTES:
                res = self._run_closest_routes(payload)
            elif payload.algorithm == AlgorithmType.RIDE_MATCHING:
                res = self._run_ride_matching(payload)
            else:
    ...
    def _run_closest_routes(self, payload: ComputeMessage) -> ClosestRoutesResult:
        ...
        try:
            ...
            return ClosestRoutesResult(
                routes=[ClosestRoute(route_id="1", distance_to_point_meters=100, access_point=params.point)]
            )
